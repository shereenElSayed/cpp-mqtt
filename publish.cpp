...
const string TOPIC_1 			{ "hello"};
const string TOPIC_2 			{ "data"};
const int	 QOS = 1;
...
int main(int argc, char* argv[])
{
	...
	mqtt::topic top_1(cli, TOPIC_1, QOS, true);
	mqtt::topic top_2(cli, TOPIC_2, QOS, true);
	...
	try {
		...
		while (true) {
			...
			top_1.publish(std::move(payload));
			top_2.publish(std::move(payload));
